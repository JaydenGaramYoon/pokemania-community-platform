...
const TalkTalk = () => {
  ...
  // userId -> name mapping
  // Convert userId to string for comparison (ObjectId compatible)
  ...
  const userIdToName = (userId) => {
    ...
    const found = profiles.find((p) => normalizeId(p.userId) === userIdStr || normalizeId(p._id) === userIdStr);
    return found ? found.name : userIdStr;
  };
  ...
  // When profiles are loaded, prefer profile.name for the logged-in username
  useEffect(() => {
    try {
      const user = localStorage.getItem('user');
      const userData = user ? JSON.parse(user) : null;
      const userId = userData?._id;
      if (!userId || !profiles || profiles.length === 0) return;
      const found = profiles.find(p => normalizeId(p.userId) === normalizeId(userId) || normalizeId(p._id) === normalizeId(userId));
      if (found && found.name) {
        setUsername(found.name);
      }
    } catch (err) {
      console.error('[Talktalk] Failed to apply profile name:', err);
    }
  }, [profiles]);
  ...
  useEffect(() => {
    const checkForUpdates = async () => {
      try {
        ...
        if (!res.ok) {
          const text = await res.text(); // in case the response is not JSON
                ...
                {editingMessage === msg._id ? (
                  ...
                  <>
                    <div style={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                      <span><strong className="username">{userIdToName(msg.sender)}</strong>: {msg.message}</span>
                      <div className="timestamp">{formatTimestamp(msg.timestamp)}</div>
                    </div>
